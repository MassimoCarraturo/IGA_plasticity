% OP_PLSU_EV: assemble the matrix A = [a(i,j)], a(i,j) = 1/2 (sigma (u_j), epsilon (v_i)).
%
...
function [K, internal_energy, eps_pl] = op_plsu_ev (spu, spv, msh, uhat, eps_pl, mu, kappa, sigma_y)
  ...
  internal_energy = zeros(spv.ndof,1);

  jacdet_weights = msh.jacdet .* msh.quad_weights;
  % jacdet_weights_mu = jacdet_weights .* mu;
  % jacdet_weights_kappa = jacdet_weights .* kappa;
  ...
  for iel = 1:msh.nel
    if (all (msh.jacdet(:, iel)))
      ...
      % jacdet_kappa_iel = reshape (jacdet_weights_kappa(:,iel), [msh.nqn,1,1]);
      ...
      % aux_val2 = bsxfun (@times, jacdet_kappa_iel, divu_iel .* divv_iel);
      % 
      ...
      values(ncounter+(1:spu.nsh(iel)*spv.nsh(iel))) = K_iel(indices);
      ncounter = ncounter + spu.nsh(iel)*spv.nsh(iel);

      % assembly vector
      internal_energy(spv.connectivity(:,iel))= internal_energy(spv.connectivity(:,iel)) + internal_energy_iel;
...
K= sparse (rows(1:ncounter), cols(1:ncounter), ...
