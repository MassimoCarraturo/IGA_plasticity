...
problem_data.geo_name = 'geo_square_3D.txt';
% Generate Output folder
problem_output.folder = '/home/blackmamba/workspace/GeoPDEs/output_admissible_test_moving_heat_source_thermomech_3D_.75param';
...
problem_data.x_begin = 2.0; %[mm]
problem_data.x_end = 3.0; %[mm]
num_hatch=5; %10;
x_path_1 = linspace(problem_data.x_begin, problem_data.x_end, n_time_steps/ num_hatch);
x_path_2 = linspace(problem_data.x_end, problem_data.x_begin, n_time_steps/ num_hatch);
...
problem_data.y_begin = 2.4;
problem_data.y_end = 2.6;
hatch_space = laser_radius;
y_path_1 = linspace(problem_data.y_begin, problem_data.y_end, n_time_steps / num_hatch);
y_path_2 = linspace(problem_data.y_begin + hatch_space, problem_data.y_end +hatch_space, n_time_steps / num_hatch);
y_path_3 = linspace(problem_data.y_begin + hatch_space * 2, problem_data.y_end +hatch_space * 2, n_time_steps / num_hatch);
y_path_4 = linspace(problem_data.y_begin + hatch_space * 3, problem_data.y_end +hatch_space * 3, n_time_steps / num_hatch);
y_path_5 = linspace(problem_data.y_begin + hatch_space * 4, problem_data.y_end +hatch_space * 4, n_time_steps / num_hatch);
...
problem_data.z_begin = 1;
problem_data.z_end = 1;
...
problem_data.f = @(x,y,z,path_x,path_y,path_z) absorbtion_coeff * 6.0*sqrt(3.0)*P/(pi * sqrt(pi) * laser_radius * laser_radius * laser_penetration_depth) * ...
...
adaptivity_data.mark_param = 0.75;% 0.5; %
...
plot_data.file_name_var = strcat(problem_output.folder, '/test_moving_heat_source_thermomech_3D_variables_%d.mat');
...
plot_data.npoints_z = 101;        %number of points x-direction in post-processing
