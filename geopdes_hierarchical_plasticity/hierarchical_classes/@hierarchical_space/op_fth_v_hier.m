% OP_FTH_V_HIER: assemble the right-hand side vector r = [r(i)], with  r(i) = (f, v_i) for hierarchical splines, 
...
function rhs = op_fth_v_hier (hspace, hmsh, f, lambda, mu)
  ...
  for ilev = 1:hmsh.nlevels
    ...
    if (hmsh.nel_per_level(ilev) > 0)
      ...
      sp_lev = sp_evaluate_element_list (hspace.space_of_level(ilev), hmsh.msh_lev{ilev}, 'value', true, 'gradient', true, 'divergence', true);
      b_lev = op_th_v (sp_lev, sp_lev, hmsh.msh_lev{ilev}, f(x{:}), lambda(x{:}), mu(x{:}));
