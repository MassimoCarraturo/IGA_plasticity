% OP_PLSU_EV_HIER: assemble the matrix A = [a(i,j)], a(i,j) = 1/2 (sigma (u_j), epsilon (v_i))
%  for hierarchical splines, exploiting the multilevel structure.
%
%   mat = op_plsu_ev_hier (hspu, hspv, hmsh, lambda, mu);
%   [rows, cols, values] = op_plsu_ev_hier (hspu, hspv, hmsh, lambda, mu);
%
% INPUT:
%
