...
for itime = 1:number_ts
    if itime==1
        num_max_iter=adaptivity_data.num_max_iter+4;
    else
        num_max_iter=adaptivity_data.num_max_iter;
    end
    ...
    if ~(isempty(find(plot_data.time_steps_to_post_process==itime, 1)))
        ...
        [eu, F] = sp_eval (u, hspace, geometry, npts,{'value'});
        %% TODO: make it nicer
        %eu{2} = eu{2}*29;
        ...
        if (plot_data.plot_hmesh)
            ...
            title_folder = sprintf(plot_data.folder_name_mesh, itime);
            mkdir(title_folder)
            title_file = sprintf(plot_data.file_name_mesh, itime);
            hmsh_to_vtk (hmsh, 2, title_file);
        ...
        if (plot_data.plot_matlab)
            ...
            % npts = [plot_data.npoints_x plot_data.npoints_y];
            % [eu, F] = sp_eval (u, hspace, geometry, npts);
            % figure(1000 + itime); surf (squeeze(F(1,:,:)), squeeze(F(2,:,:)), eu{1}(:,:,end))
            % title = sprintf(plot_data.file_name, itime);
            % saveas(gcf,title);
        end
    end
