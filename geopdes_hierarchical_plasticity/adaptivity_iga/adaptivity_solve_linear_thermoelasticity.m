% ADAPTIVITY_SOLVE_LINEAR_ELASTICITY: Solve a linear thermoelasticity problem with hierarchical splines.
%
...
function [u,int_enrg] = adaptivity_solve_linear_thermoelasticity (hmsh, hspace, problem_data)

mat    = op_su_ev_hier (hspace, hspace, hmsh, problem_data.lambda_lame, problem_data.mu_lame); 
rhs_load = op_f_v_hier (hspace, hmsh, problem_data.f);
rhs    = op_fth_v_hier (hspace, hmsh, problem_data.fth, problem_data.lambda_lame, problem_data.mu_lame);
...
int_enrg = 0.5*u(int_dofs)'*mat(int_dofs, int_dofs)*u(int_dofs);
