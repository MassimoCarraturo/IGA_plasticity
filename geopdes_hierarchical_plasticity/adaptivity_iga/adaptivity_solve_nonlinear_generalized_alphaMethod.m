...
function [u, int_enrg, problem_data] = adaptivity_solve_nonlinear_generalized_alphaMethod (hmsh, hspace, time_step, problem_data, plot_data, u_0)
...
while (norm(res) > problem_data.Newton_tol)
    ...
    stiff_mat = op_gradu_gradv_hier_nl (hspace, hspace, hmsh, problem_data.c_diff, u);
    mass_mat = op_u_v_hier_nl(hspace, hspace, hmsh, problem_data.c_cap, u, u_0);
    %lumped mass matrix
...
int_enrg = 0.5*u(int_dofs)'*stiff_mat(int_dofs, int_dofs)*u(int_dofs);
