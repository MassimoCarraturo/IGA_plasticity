...
[ders2, ~] = hspace_eval_hmsh (u, hspace, hmsh, 'hessian');
[hdiv, F] = hspace_eval_hmsh (u, hspace, hmsh, 'divergence');
%[hgrad, F] = hspace_eval_hmsh (u, hspace, hmsh, 'gradient');
...
eth = problem_data.fth (x{:});

load_th = bsxfun(@times, 2*mu*eth + lambda*repmat(sum(eth,1),[hmsh.ndim,1,1]), reshape(hdiv',[1,size(hdiv)])); %dot(hgrad,reshape(repmat(2*mu*eth + lambda*repmat(sum(eth,1),[hmsh.ndim,1,1]),[1,1,hmsh.ndim]),size(hgrad)), 1); %
valf = problem_data.f (x{:}) + load_th;%reshape(sum(load_th,1),size(problem_data.f (x{:})));
